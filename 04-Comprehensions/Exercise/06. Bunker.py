if __name__ == "__main__":
    ...
    categories = [
        {"category": food_category, "foods": []}
        for food_category in categories_string
    ]
    total_qty = 0
    total_qlty = 0
    N = int(input())
    ...
    print(
        f"Count of items: {total_qty}\nAverage quality: \
        {total_qlty/len(categories_string):.2f}"
    )
    print(
        "\n".join([f"{x['category']} -> {', '.join(['foods'])}"
                   for x in categories])
    )
